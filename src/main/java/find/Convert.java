...
public class Convert {
    ...
    public void convertDate(@NotNull WebDriver driver, String oldDateString) throws ParseException {
...
//добавить в getText по дате выборку только валидных дат, чтобы убрать блок ниже
        map.entrySet()
        ...
        for (Map.Entry<String, Date> entry : map1.entrySet()) {
            for (Date date : od) {
                if (entry.getValue().toString().equals(String.valueOf(date))) {
                    System.out.println("Курс: " + entry.getKey() + " " + "старует " + entry.getValue().toString().replace("1970","2022"));
                }
            }
        }
    }
